...
public class TransactionPool {
    ...
    // TODO: change this to 100 for release
    private static final int maximumTransactionsInPoolPerSender = 10;
        ...
        if (transactionBlockHeight <= frozenEdgeHeight) {
            ...
            if (BalanceListManager.accountIsInSystem(transaction.getSenderIdentifier())) {
                ...
                if (transactionsForHeight.keySet().contains(signature)) {
                    ...
                    // TODO: This statement is for testing only. Remove this.
                    warning.append("After adding transaction, sender ").
                            append(PrintUtil.compactPrintByteArray(senderIdentifier.array())).append(" has ")
                            .append(senderCountMap.get(senderIdentifier)).append(" transactions in the system. ");
    ...
    public static synchronized void updateFrozenEdge() {
        ...
        // TODO: This loop is for testing only. Remove this.
        for (ByteBuffer senderIdentifier : TransactionPool.senderCountMap.keySet()) {
            System.out.println("[TransactionPool] " + PrintUtil.compactPrintByteArray(senderIdentifier.array()) + ": " +
                    TransactionPool.senderCountMap.get(senderIdentifier));
        }
    }
}
